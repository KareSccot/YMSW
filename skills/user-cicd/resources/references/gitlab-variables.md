...
如果应用只跑 HTTP（上游 LB / Ingress 终止 SSL），**留空即可**——`vm-deploy.yml` scp 空文件到远端不报错。详见 `resources/references/ssl-cert.md`。
...
SSH key / SSL 证书同环境各节点共用一份（同运维域、同域名时）。多节点负载均衡部署超出本 skill 范围，转交平台工程师。
...
`docker-compose.yml` 的 ports 行用两个独立变量：`"${CUSTOM_HOST_PORT:-X}:${CUSTOM_APP_PORT:-Y}"`。详见 `resources/references/ssl-cert.md § G`。
...
| **`CUSTOM_DATA_DIR`** | **host 数据目录的 bind mount 路径覆盖** | ☐ | 仅在 compose 用 `${CUSTOM_DATA_DIR:-...}:/app/data` 时配；典型值 `/home/appdeploy/<svc>-data` 或 `/var/lib/<svc>`。详见 `resources/references/data-persistence.md`。**注意**：VM 端配完变量后仍需一次性 `sudo mkdir -p <值> && sudo chown 1000:1000 <值>`。 |
